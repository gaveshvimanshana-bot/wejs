...
// ==========================
// 🔥 PLUGIN AUTO LOADER (ADDED)
// ==========================
const repo = "https://github.com/gaveshvimanshana-bot/One";

if (!fs.existsSync("./plugins")) {
console.log("📥 Downloading plugins...");
try {
execSync(git clone ${repo} plugins);
} catch (e) {
console.log("❌ Plugin download failed:", e.message);
}
} else {
  ...
  if (cmd) {    
    ...
    try {    
      cmd.function(hansa, mek, m, {    
        ...
        args, q, isGroup, senderNumber, reply    
      });    
    } catch (e) {    
...
for (const handler of replyHandlers) {    
  if (handler.filter(body, { sender, message: mek })) {    
    try {    
      await handler.function(hansa, mek, m, { from, body, reply });    
      break;    
    } catch (e) {    
